...
int validate_board(char board[ROWS][COLUMNS]) {
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLUMNS; j++) {
            if ( board[i][j] != 'X' && board[i][j] != 'O' && (board[i][j] < '0' || board[i][j] > '9') ) {
                printf("[ERROR] Invalid board!  %c is invalid\n", board[i][j]);
                return 0;
            } 
            if ( board[i][j] == 'X' ) {
                numOfX++;
    ...
    if (numOfX != numOfO + 1) {
        printf("[ERROR] Invalid board! number of X should be number of O + 1\n");
        return 0;
    }
    return 1;
}
                ...
                if (rc == 0) {
                    ...
                    if (recv_data->command == NEWGAME) {
                        ...
                        if (game_number == -1) { // all slots are full
                            ...
                            copy_board(send_data->board, boards[game_number]->board);
                            send_protocol(clientSDList[i], send_data);
                        }
                    } else if (recv_data->command == CONTINUE) {
                        // find an available slot
                        int game_number = get_valid_board(boards);
                        if (game_number == -1) { // all slots are full
                            send_data->command = CONTINUE;
                            send_data->move = -1;
                            send_data->game_no = FULL;
                            send_protocol(clientSDList[i], send_data);
                        } else { // get a game number
                            ...
                            if (!validate_board(recv_data->board)) {
                                send_data->command = CONTINUE;
                                send_data->move = -1;
                                send_data->game_no = FULL;
                                send_protocol(clientSDList[i], send_data);
                            ...
                            // get the board
                            copy_board(boards[game_number]->board, recv_data->board);
...
int tictactoe_client(int sockfd, int player, struct sockaddr_in from_address, int flag, char origin_board[ROWS][COLUMNS]) {
    ...
    if (flag == 0) { // new game
        ...
        copy_board(send_data->board, board);
        ...
        printf("[INFO] Sent a continue request.\n");
    }

    print_board(send_data->board);
    ...
    while(1) {
        ...
        //    exit(1);
        //}
        //if server is down, send CONTINUE request
        if (rc == -1 || rc == 0) {
            printf("Server is down. Send CONTINUE requst.\n");
            // send_data->command = CONTINUE;
            // send_data->move = -1; // does not matter, client does not make move at this time
            // send_data->game_no = -1; // does not matter, the game number would be assigned by server
            // copy_board(send_data->board, board);
            ...
            // copy the current board to origin board
            copy_board(origin_board, board);
            return CRASH;
        }
        ...
        if (recv_data->game_no == FULL) { // game slot full
        ...
        } else if (recv_data->command == MOVE || recv_data->command == NEWGAME || recv_data->command == CONTINUE) { 
            ...
            if (recv_data->command == MOVE || recv_data->command == CONTINUE) {
...
void serialize_protocol(char* buffer, protocol* data) {
    ...
    for (int i = 0; i < BOARDSIZE; i++) {
        memcpy(buffer+12+i, &(data->board[i/ROWS][i%COLUMNS]), 1);
    }
...
void deserialize_protocol(char* buffer, protocol* data) {
    ...
    for (int i = 0; i < BOARDSIZE; i++) {
        memcpy(&(board[i/ROWS][i%COLUMNS]), buffer+12+i, 1);
        memcpy(&(data->board[i/ROWS][i%COLUMNS]), buffer+12+i, 1);
    }
...
int recv_protocol(int sockfd, protocol* data) {
    char receiveBuffer[PROTOCOLSIZE];
